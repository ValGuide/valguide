...
import { ChevronUp, Share, Globe, Heart } from 'lucide-react'
  ...
  return (
    <Drawer.Root shouldScaleBackground open={isOpen} onOpenChange={onOpenChange}>
      <Drawer.Trigger asChild>
        <button className="absolute bottom-0 left-0 right-0 flex flex-col items-center pb-8 pt-4 text-white/50 hover:text-white/80 transition-colors z-20">
...
function ActionButton({ icon, label }: { icon: React.ReactNode, label: string }) {
    return (
        <button className="flex flex-col items-center gap-2 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">

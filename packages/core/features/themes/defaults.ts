...
export function generateThemeScript(cookieName: string): string {
  ...
  return `(function(){var m=document.cookie.match(/${cookieName}=([^;]+)/),t=m?m[1]:'system',r;r=t==='system'?(window.matchMedia('(prefers-color-scheme:dark)').matches?'${darkTheme}':'${lightTheme}'):t==='light'?'${lightTheme}':t==='dark'?'${darkTheme}':t;document.documentElement.setAttribute('data-theme',r)})()`.replace(
    /\n/g,
    '',
  )
}

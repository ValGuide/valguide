...
export const qrBrandingOverrideSchema = z.object({
  ...
  bgColor: backgroundColorSchema.optional(),
...
export function sanitizeQrBrandingOverride(value: unknown): QrBrandingOverride {
  ...
  const fgColor = hexColorSchema.safeParse(raw.fgColor)
  if (fgColor.success) sanitized.fgColor = fgColor.data

  const bgColor = backgroundColorSchema.safeParse(raw.bgColor)
...
export function sanitizeBackgroundColor(value: string | undefined, fallback: string): string {
  if (!value) return fallback
  return backgroundColorSchema.safeParse(value).success ? value : fallback
}
...
export function resolveQrBranding(input: {
  ...
  const effective: QrBrandingConfig = {
    ...
    bgColor: sanitizeBackgroundColor(
  ...
  return {
    ...
    hasContrastWarning:
      normalized.bgColor === 'transparent' ? false : !hasGoodContrast(normalized.fgColor, normalized.bgColor),
  }
}
...
export function applyQrOverrideToBranding(
  ...
  const bgColor = sanitizeBackgroundColor(sanitized.bgColor, base.bgColor)
  ...
  return {
    ...
    hasContrastWarning: bgColor === 'transparent' ? false : !hasGoodContrast(fgColor, bgColor),
  }
}

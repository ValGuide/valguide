...
export function PostgresDrizzleAdapter(
  ...
  const { usersTable, accountsTable, sessionsTable, verificationTokensTable, authenticatorsTable } = schema

  return {
    async createUser(data: AdapterUser) {
      const { id, ...insertData } = data
      const hasDefaultId = getTableColumns(usersTable)['id']['hasDefault']
      const username = null // TODO:  await createInitialUsername(data.email)
      return client
        .insert(usersTable)
        .values({ ...insertData, username, ...(hasDefaultId ? {} : { id }) })
        .returning()
    ...
    async updateUser(data: Partial<AdapterUser> & Pick<AdapterUser, 'id'>) {
      ...
      const [result] = await client.update(usersTable).set(data).where(eq(usersTable.id, data.id)).returning()

...
export function AssetsList({
  ...
  const handleUploadClick = (type: AssetType) => {
    // Mock for Storybook - just trigger callback
    onUploadComplete?.({
      id: 'new-asset',
      nanoId: 'new',
      fileName: `new-${type}.${type === 'image' ? 'jpg' : type === 'audio' ? 'mp3' : 'mp4'}`,
      fileSize: 1024,
      ...
      {filteredAssets.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredAssets.map((asset) => (
            <AssetCard key={asset.id} asset={asset} onDelete={onAssetDeleted} onPreview={onPreview} />
          ))}
        </div>
      )}
    </div>

...
import type { GuideWithStops, StopWithTranslations, GuideTranslation, StopTranslation } from '@valguide/core/features/guides/schema'
import type { Asset } from '@valguide/core/features/assets/schema'
import type { SupportedLocale } from '@valguide/i18n/i18n.config'
import { updateGuide, updateGuideTranslation, createStop, updateStop, deleteStop as deleteStopAction, reorderStops as reorderStopsAction, attachAssetToStop as attachAssetToStopAction, detachAssetFromStop as detachAssetFromStopAction, attachAssetToGuide as attachAssetToGuideAction, detachAssetFromGuide as detachAssetFromGuideAction } from '@valguide/core/features/guides/actions'
...
interface GuideEditorContextValue {
  ...
  // Guide actions
  updateGuideTranslationData: (locale: SupportedLocale, data: Partial<Omit<GuideTranslation, 'id' | 'guideId' | 'createdAt' | 'updatedAt'>>) => void
  ...
  updateStopTranslationData: (stopId: string, locale: SupportedLocale, data: Partial<Omit<StopTranslation, 'id' | 'stopId' | 'createdAt' | 'updatedAt'>>) => void
...
export function GuideEditorProvider({ 
  ...
  const updateGuideTranslationData = useCallback((locale: SupportedLocale, data: Partial<Omit<GuideTranslation, 'id' | 'guideId' | 'createdAt' | 'updatedAt'>>) => {
    setGuide(prev => {
  ...
  const updateStopTranslationData = useCallback((stopId: string, locale: SupportedLocale, data: Partial<Omit<StopTranslation, 'id' | 'stopId' | 'createdAt' | 'updatedAt'>>) => {
    setGuide(prev => ({
      ...prev,
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      for (const translation of guide.translations) {
  ...
  return (
    <GuideEditorContext.Provider value={value}>
      {children}

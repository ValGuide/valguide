...
export function SharedStopBanner({ guideCount }: SharedStopBannerProps) {
  const t = useTranslations('stops.editor')

  if (guideCount <= 1) return null
    ...
    <div className="flex items-center gap-3 rounded-lg border border-warning/20 bg-warning/5 p-3 text-sm">
      ...
      <div className="flex flex-1 flex-wrap items-center gap-x-2 gap-y-1">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">{t('usedInGuides', { count: guideCount })}</span>
        </div>
